...
to `.coraline/models/<model>/`.

### `model_file`

Pin a specific ONNX filename (e.g. `model_int8.onnx`). When unset, Coraline
auto-detects the first file from the active model's preference order.
...
| `jina-embeddings-v2-base-code` | 768 | `model_quantized.onnx` (~162 MB) | Code-specialised. Trained on 150M+ code/docstring pairs across 30+ languages. 8192-token context via ALiBi. |
...
```bash
coraline model download   # fetches jina tokenizer + ONNX
coraline embed --reembed  # regenerates every node under the new model
```

`coraline_semantic_search` automatically filters by the active model — old
`nomic-embed-text-v1.5` rows remain in the SQLite `vectors` table tagged
